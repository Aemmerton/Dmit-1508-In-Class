...
--10. Which student(s) have the highest average mark? Jint - This can only be done by a subquery

-- List the average marks of the students; this will be teh subquery portion
SELECT AVG(Mark) AS 'Average'
FROM	Registration		
GROUP BY StudentID
-- List the students whose average mark is as large as teh largest average.
...
HAVING AVG(Mark) >= ALL (SELECT AVG(Mark) AS 'Average' FROM	Registration WHERE Mark IS NOT NULL GROUP BY StudentID)
